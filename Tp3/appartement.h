...
const std::string ADRESSE = "1 rue Joliot-Curie 91400 Orsay";
const std::vector<int> ZONES = {2500, 3100, 3500, 4150, 4500};
using namespace std;
...
class Appartement
{
    std::vector<Piece> _lesPieces;
    std::string _adresse;

public:
    //---------------  constructeurs
    Appartement(); /// appartement vide au départ
    Appartement(const Piece &arg, const std::string &); // un appartement avec une piece et une adresse
    ...
    int recherche(const std::string &nomPiece) const;
    int recherchePremier(const std::string &nomPiece) const;
};
#endif
